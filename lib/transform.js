...
function getFileSpecificHeaderLevel(content) {
    var maxlevelRE = /<\!--\sDOCTOC\smaxlevel\s(\d)\s-->/;
    var match = content.match(maxlevelRE);
    if (match) {
...
exports = module.exports = function transform(content, mode, maxHeaderLevel, title, notitle, entryPrefix) {
  ...
  maxHeaderLevel = getFileSpecificHeaderLevel(content) || maxHeaderLevel;
